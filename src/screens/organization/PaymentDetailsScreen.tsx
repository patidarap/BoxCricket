...
export const PaymentDetailsScreen = ({ route, navigation }: any) => {
  ...
  const paidAmount = paidMembers * payment.perPersonAmount;
  const leftAmount = payment.amount - paidAmount;
  const progressPercentage = (paidAmount / payment.amount) * 100;

  const handleVerifyPayment = async (splitMemberId: string, status: 'done' | 'pending') => {
    try {
      const response = await organizationService.verifyPayment(splitMemberId, status);
      if (response.success) {
        showSuccessMessage(status === 'done' ? 'Payment approved' : 'Payment cancelled');
        setMembers((prev: any[]) =>
          prev.map((m: any) =>
            m.splitMemberId === splitMemberId ? { ...m, status } : m
          )
        );
      } else {
          ...
          {isAdmin && status === 'paid' && (
            <View style={styles.actionButtons}>
              ...
              <TouchableOpacity
                ...
                onPress={() => handleVerifyPayment(item.splitMemberId, 'pending')}
              >
    ...
    <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      <ScrollView style={styles.content}>
        <View style={[styles.totalSection, { backgroundColor: theme.colors.surface }]}>
          <Avatar name={payment.createdBy?.name || 'User'} size="md" />
          <Spacer size="md" />
